...
public class ProcesManager {
    ...
    public void delete_proces(String name) throws Exception {//NO I TUTAJ MILA COŚ JEST ŻLE//jeśli to czytacie znaczy już jest ok
        ...
        int w=0;//flaga "czy znaleziono" taki proces
            ...
            Iterator<Proces> iterr = all_processes.iterator();
                while (iterr.hasNext()) {
                    Proces p = iterr.next();
                    if (p.get_name().equals(name)) iterr.remove();
                    
            if(p.get_name().equals(name)){
                int czy_czarek_usuwa=0;
                if(p.get_state()==0||p.get_state()==3||p.get_state()==2) czy_czarek_usuwa=1;
    ...
    public int czy_istnieje(String nazwa){
        for(Proces p:all_processes){
            if(p.get_name().equals(nazwa)) return 1;
        }
        return 0;
    }
    ...
    public Stronica[] get_page_table(String name){
        for(Proces p:all_processes){
            if(p.get_name().equals(name)) return p.get_page_table();
        }
        return null;
    }
    ...
    public void show_all(){
        for(Proces p: all_processes){
            if(p.get_state()==3&&p.get_ID()!=0) System.out.println(p.show_info());
        }
        for(Proces p: all_processes){
            if(p.get_state()==0&&p.get_ID()!=0) System.out.println(p.show_info());
        }
        for(Proces p: all_processes){
            if(p.get_state()==1&&p.get_ID()!=0) System.out.println(p.show_info());
        }
    }
