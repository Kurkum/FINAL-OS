   ...
   public String name;
	...
	public void setState(boolean state)  {
       ...
       waitingProcesses.offer(process);
        ...
        public void releaseProcess(String nazwa)
        {
            for(Iterator<Proces> it = waitingProcesses.iterator(); it.hasNext();)
            {
                Proces pro = it.next();
                if(pro.get_name().equals(nazwa))
                {
                    try {
                        manager.set_state(pro.get_ID(),0);
                        pro.set_if_lock(0);
                    } catch (Exception ex) {
                       // Logger.getLogger(MutexLocks.class.getName()).log(Level.SEVERE, null, ex);
                    }
