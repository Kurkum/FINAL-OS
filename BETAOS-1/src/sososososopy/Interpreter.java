...
/**
 *
 * @author Jedrzej Magdans
 * AD - dodawanie wartości rejestrów
 * SB - odejmowanie wartości rejestrów
 * AX - dodawanie liczby do rejestru
 * MO - wstawienie liczby do rejestru
 * IN - zwiększenie wartości rejestru o 1
 * DC - zmniejszenie wartości rejestru o 1
 * MU - mnożenie wartości rejestrów
 * MX - Mnożenie rejestru przez podaną liczbę
 * MV - zmiana wartości rejestru x na wartość rejestru y
 * JP - skok
 * JE- skok dopóki rejA!=0
 * MF- tworzenie pliku
 * AF- dopisywanie do pliku
 * EF- edycja pliku
 * DF-usuwanie pliku
 * CX - wysyłanie wiadomości
 * CD- odczytywanie wiadomości ilość znaczków pobrana z rozkazu
 * CL- -             ||                               - rejestru
 * UP- tworzenie kolejki
 * DP- usuwanie kolejki
 * NP- rozkaz pusty
 * WM - zapisywanie do rejestrów
 * HL-koniec programu
 * SR- zapis rozkazu do ramu
 * ZR - zapis do ramu z rejestru
 */
           ...
           case "MF":
            //    System.out.println("tworzenie i zapisywanie pliku");
               ...
               // disc.pokazZamki(); // lockijii ------------------------------------------------------------------------------------
                ...
                case"ZR":
                         if (rozkaz[1].equals("A")) {
                String wiadomosc2 =  Integer.toString(running.running.R1);
                for(int i=0; i<wiadomosc2.length(); i++){
                char c = wiadomosc2.charAt(i);
                     zarz.writeMemory(Integer.parseInt(rozkaz[2])+i,c);
                }
                manager.increase_counters();
            } else if (rozkaz[1].equals("B")) {
              String wiadomosc2 =  Integer.toString(running.running.R2);
                for(int i=0; i<wiadomosc2.length(); i++){
                char c = wiadomosc2.charAt(i);
                     zarz.writeMemory(Integer.parseInt(rozkaz[2])+i,c);
                }
                ...
                String wiadomosc2 =  Integer.toString(running.running.R3);
                for(int i=0; i<wiadomosc2.length(); i++){
                char c = wiadomosc2.charAt(i);
                     zarz.writeMemory(Integer.parseInt(rozkaz[2])+i,c);
                }
                ...
                String wiadomosc2 =  Integer.toString(running.running.R4);
                for(int i=0; i<wiadomosc2.length(); i++){
                char c = wiadomosc2.charAt(i);
                     zarz.writeMemory(Integer.parseInt(rozkaz[2])+i,c);
                }
